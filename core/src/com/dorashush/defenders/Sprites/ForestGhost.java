...
public class ForestGhost extends Enemy{
    ...
    private ForestGhost.State currentState,previousState;
    ...
    public ForestGhost.State getState(){
        shootingTimer%=7;

        if(previousState == ForestGhost.State.FIREING && shootingTimer>0.02 &&shootingTimer<1.2){
            return ForestGhost.State.FIREING;
        }
        else if(shootingTimer <=0.02 && avoidFirstHitTimer>2){
            return ForestGhost.State.FIREING;
        }
        else{
            return ForestGhost.State.WALKING;
        }
    }
    ...
    public void movment(float dt){
        if (!gotHit) {
            ...
            if(getState()==ForestGhost.State.FIREING){
